...
- `src/format/` — TOON output rendering (`toon.ts`): TOON by default, `TOON_TRU_BIN` external-binary pipe with in-process fallback.
...
- CLI stdout (`ask`/`deep`/`auth-status`) is TOON by default; `--json` (anywhere in argv) selects JSON. The Hermes adapter appends `--json` itself because its contract is `json.loads` on stdout.
- MCP tools serialize success payloads as TOON by default; `format: "json"` parameter opts into JSON. Errors always stay human-readable text, never TOON/JSON.
- `TOON_TRU_BIN` pipes the JSON payload through an external binary (JSON stdin → TOON stdout); failure/timeout falls back to the in-process `@toon-format/toon` encoder.
- `npm install` rewrites `package-lock.json` and drops the manually backfilled `integrity` hashes on the three nested dev-only `@earendil-works/*` entries; re-backfill them before `nix build` or the FOD panics.
